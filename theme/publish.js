  ...
  if (members.globals.length) {
    ...
    groupList.forEach(groupName => {
      members.globals.forEach(item => {
        const hasType = item.type && item.type.names;
        const hit = hasType ? !!~item.type.names.join('').indexOf(groupName) : !!~item.longname.toLowerCase().indexOf(groupName);
        if (hit) {
...
exports.publish = function(taffyData, opts, tutorials) {
  ...
  const distFilePath = path.join(__dirname, '..', 'docs', 'index.html');
  let distFileContent = fs.readFileSync(distFilePath, 'utf8');
  const dom = new JSDOM(distFileContent);
  const { document } = dom.window;
  const prefix = document.querySelector('link[rel="canonical"]').href;
