...
const RUN_IN_PARRELLEL = !!process.env.RUN_IN_PARRELLEL;

const uuidForParreller = _.uuid();
...
const extendsMixIn = wd => {
  ...
  wd.addPromiseChainMethod('getUrl', function(url) {
    const p = RUN_IN_PARRELLEL ? this.setCookie({
      url,
      name: 'DATAHUB_CACHE_TAG',
      value: uuidForParreller,
    }) : this;
    return p
      .get(url)
