...
const { opn, platform, isExistedFile, uuid, mkdir } = _;
...
const extendsMixIn = wd => {
  ...
  wd.addPromiseChainMethod('getUrl', function(url) {
    const p = MACACA_RUN_IN_PARALLEL ?
      this.setCookie({
        url,
          ...
          return new Promise(resolve => {
            const handle = () => {
              setTimeout(() => {
                this.get(url)
                  .execute(
                    'return document.querySelectorAll(".root-app").length'
                  )
                  .then(number => {
                    if (number !== 0) {
                      resolve();
  ...
  wd.addPromiseChainMethod('getUrlByExecuteScripts', function(
    url,
    scriptStr,
    successHandle
  ) {
  ...
  wd.addPromiseChainMethod('startMediaRecorder', function() {
    return this.execute('window._startMediaRecorder();');
  });
