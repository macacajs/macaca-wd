...
const extendsMixIn = wd => {
  ...
  wd.addPromiseChainMethod('formatInput', function(selector, string) {
    const uuid = Date.now();
    const input = `document.querySelector('${selector}')`;
    const eventHandle = `var event = document.createEvent('Event');event.initEvent('input', true, true);_element_${uuid}.dispatchEvent(event);`;
    ...
    if (list.length === 1) {
      const script = `var _element_${uuid} = ${input};_element_${uuid}.value='${value}';${eventHandle}`;
      return this
        .execute(script)
        .sleep(100);
    }
