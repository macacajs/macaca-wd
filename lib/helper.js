...
const extendsMixIn = wd => {
   ...
   * @returns {Promise.<boolean>}
   */
  wd.addPromiseChainMethod('hasElement', function (cssSelector) {
    return this
  ...
  /**
   * Check if element not existed.
   * @name elementNotExist
   * @summary Support: Web(WebView)
   * @param {string} css selector.
   * @type assert
   * @returns {Promise.<boolean>}
   */
  wd.addPromiseChainMethod('elementNotExist', function (cssSelector) {
    return this
      .elementByCss(cssSelector)
      .then(() => {
        throw new Error(`Element ${cssSelector} should not existed.`);
      }, () => {});
  });
   ...
   * @returns {Promise.<boolean>}
   */
  wd.addPromiseChainMethod('assertAttribute', function (cssSelector, attribute, value) {
    return this
      .execute(`return document.querySelector('${cssSelector}').getAttribute('${attribute}')`)
      .then(realValue => {
        assert(value === realValue, `expect ${cssSelector}'s attribute ${attribute} to be '${value}' but got '${realValue}'`);
      });
  });
};
